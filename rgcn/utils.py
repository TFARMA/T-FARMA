...
def r2e(triplets, num_rels, bid_num=1):
...
def build_sub_graph(num_nodes, num_rels, triples, use_cuda, gpu, bid_num=1):
    def comp_deg_norm(g):
    ...
    g.uniq_r, g.r_len, g.r_to_e = r2e(triples, num_rels, bid_num)
    if use_cuda:
        g.to(gpu)
        g.r_to_e = torch.from_numpy(np.array(g.r_to_e)).long()
    return g
...
def split_by_time(data):
    ...
    times.sort()
    print(
        "# Sanity Check:  ave node num : {:04f}, ave rel num : {:04f}, snapshots num: {:04d}, max edges num: {:04d}, "
        "min edges num: {:04d}, max union rate: {:.4f}, min union rate: {:.4f}".format(
        ...
        for index in top_indices[_]:
            ...
            if r < num_rels:
                predict_triples.append(
                    [test_triples[_][0], r, index, test_triples[_][3]]
                )
            else:
                predict_triples.append(
                    [index, r - num_rels, test_triples[_][0], test_triples[_][3]]
                )
